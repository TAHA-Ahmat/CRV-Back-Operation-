...
const volSchema = new mongoose.Schema({
  ...
  // TYPE D'OPÉRATION — DÉDUIT AUTOMATIQUEMENT (Cahier des charges §3)
  // Règle métier : le type n'est JAMAIS choisi à l'avance, il est CONSTATÉ
  // - Données arrivée uniquement → ARRIVEE
  // - Données départ uniquement → DEPART
  // - Données arrivée ET départ → TURN_AROUND (déduit, jamais imposé)
  typeOperation: {
    type: String,
    enum: ['ARRIVEE', 'DEPART', 'TURN_AROUND', null],
    default: null
    // SUPPRIMÉ : required: true — le type est déduit, pas imposé
  },
  compagnieAerienne: {
    type: String,
    required: true,
